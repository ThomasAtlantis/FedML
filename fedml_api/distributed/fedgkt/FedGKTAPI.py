...
def FedML_FedGKT_distributed(process_id, worker_number, device, comm, model,
...
def init_server(args, device, comm, rank, size, model):
    ...
    logging.info("INITED GKTServerTrainer")
    # start the distributed training
    server_manager = GKTServerMananger(args, server_trainer, comm, rank, size)
    logging.info("INITED GKTServerMananger")
    server_manager.run()
...
def init_client(args, device, comm, process_id, size, model, train_data_local_dict, test_data_local_dict):
    ...
    trainer = GKTClientTrainer(client_ID, train_data_local_dict, test_data_local_dict, device, model, args)
